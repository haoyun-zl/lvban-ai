...
export async function POST(request:Request){
 try{
  ...
  const pois=(curatedPlaces(city)||await places(geo.lat,geo.lon)).filter(p=>!isTransit(p)&&(geo.countryCode!=="cn"||containsChinese(p.name)));
...
function containsChinese(text:string){return/[\u3400-\u9fff]/.test(text)}
...
function curatedPlaces(city:string){
 ...
 if(["哈尔滨","harbin"].includes(key)){
  const harbin:Array<[string,number,number,string]>=[
   ["中央大街",45.7713,126.6167,"历史街区"],
   ["圣索菲亚教堂",45.7681,126.6216,"建筑地标"],
   ["哈尔滨防洪胜利纪念塔",45.7795,126.6158,"城市地标"],
   ["斯大林公园",45.7812,126.6131,"滨江公园"],
   ["太阳岛风景区",45.7969,126.5956,"自然景区"],
   ["哈尔滨冰雪大世界",45.8167,126.5714,"冰雪主题景区"],
   ["黑龙江省博物馆",45.7576,126.6331,"历史博物馆"],
   ["哈尔滨大剧院",45.8421,126.6398,"现代建筑"],
   ["中华巴洛克历史文化街区",45.7830,126.6423,"历史街区"],
   ["果戈里大街",45.7553,126.6482,"特色街区"],
   ["侵华日军第七三一部队罪证陈列馆",45.6262,126.6320,"历史纪念馆"],
   ["东北虎林园",45.8189,126.6087,"野生动物园"]
  ];
  return harbin.map(([name,lat,lon,label],i)=>({id:`harbin-${i}`,name,lat,lon,category:"attraction",tags:{source:"city-core",label}}));
 }
 ...
 if(["韩国","南韩","south korea","korea","대한민국"].includes(key)){
  const korea:Array<[string,string,number,number,string]>=[
   ["景福宫","경복궁",37.5796,126.9770,"历史宫殿"],
   ["北村韩屋村","북촌한옥마을",37.5826,126.9831,"传统街区"],
   ["昌德宫","창덕궁",37.5794,126.9910,"世界文化遗产"],
   ["仁寺洞","인사동",37.5740,126.9858,"文化街区"],
   ["明洞","명동",37.5636,126.9869,"购物街区"],
   ["南山首尔塔","N서울타워",37.5512,126.9882,"城市观景台"],
   ["东大门设计广场","동대문디자인플라자",37.5665,127.0092,"建筑地标"],
   ["清溪川","청계천",37.5690,126.9784,"城市河道"],
   ["国立中央博物馆","국립중앙박물관",37.5239,126.9803,"国家博物馆"],
   ["广藏市场","광장시장",37.5700,126.9996,"传统市场"],
   ["梨花洞壁画村","이화벽화마을",37.5779,127.0079,"艺术街区"],
   ["盘浦汉江公园","반포한강공원",37.5100,126.9958,"滨江公园"]
  ];
  return korea.map(([zh,local,lat,lon,label],i)=>({id:`korea-${i}`,name:`${zh}（${local}）`,lat,lon,category:"attraction",tags:{source:"country-core",label}}));
 }
