...
export async function GET(request:Request){
 ...
 const lat=Number(u.searchParams.get("lat")),lon=Number(u.searchParams.get("lon")),hasCoordinates=Number.isFinite(lat)&&Number.isFinite(lon);
 if(!query)return placeholder("暂无地标图片");
 const candidates=hasCoordinates
  ?[...await exactWiki("zh.wikipedia.org",query,lat,lon),...await exactWiki("en.wikipedia.org",query,lat,lon)]
  :[...await exactWiki("zh.wikipedia.org",query),...await exactWiki("en.wikipedia.org",query),...await exactCommons(query)];
...
async function exactWiki(host:string,title:string,lat?:number,lon?:number){try{const api=`https://${host}/w/api.php?action=query&redirects=1&titles=${encodeURIComponent(title)}&prop=pageimages%7Ccoordinates&piprop=thumbnail&pithumbsize=1400&format=json&origin=*`;const r=await fetch(api,{headers});if(!r.ok)return[];const data=await r.json() as {query?:{pages?:Record<string,{missing?:string;thumbnail?:{source?:string};coordinates?:Array<{lat:number;lon:number}>}>}};return Object.values(data.query?.pages??{}).flatMap(p=>{if(p.missing||!p.thumbnail?.source)return[];if(lat===undefined||lon===undefined)return[p.thumbnail.source];const c=p.coordinates?.[0];return c&&distanceKm(lat,lon,c.lat,c.lon)<=20?[p.thumbnail.source]:[]})}catch{return[]}}
function distanceKm(aLat:number,aLon:number,bLat:number,bLon:number){const r=6371,toRad=(n:number)=>n*Math.PI/180,dLat=toRad(bLat-aLat),dLon=toRad(bLon-aLon);const h=Math.sin(dLat/2)**2+Math.cos(toRad(aLat))*Math.cos(toRad(bLat))*Math.sin(dLon/2)**2;return 2*r*Math.asin(Math.sqrt(h))}
